...
async function getExistingVote(userId, postId) {
  try {
    const { rows: existingVotes } =
      await sql`SELECT * FROM diditvotes WHERE user_id = ${userId} AND post_id = ${postId} LIMIT 1`;
...
async function handleVote(userId, postId, newVote) {
  ...
  if (existingVote) {
    if (existingVote.vote === newVote) {
      ...
      await sql`DELETE FROM diditvotes WHERE id = $1`, [existingVote.id];
    } else {
      // Update the existing vote
      await sql`UPDATE diditvotes SET vote = $1 WHERE id = $2`,
        [newVote, existingVote.id];
    }
  } else {
    // Insert a new vote
    await sql`INSERT INTO diditvotes (user_id, post_id, vote, vote_type) VALUES ($1, $2, $3, 'post')`,
